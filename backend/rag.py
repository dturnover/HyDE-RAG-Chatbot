...
try:
    ...
    if pinecone_index_name not in pc.list_indexes().names():
         raise ValueError(f"Pinecone index '{pinecone_index_name}' not found. Please ensure it's created and the name/API key are correct for the environment.")
...
def get_embedding(text, model="text-embedding-3-small"):
    ...
    if not client: # Checks the module-level 'client' variable
        logging.error("OpenAI client not initialized. Cannot get embedding.")
        return None
    try:
        text = text.replace("\n", " ")
        response = client.embeddings.create(input=[text], model=model) # Uses 'client'
...
def pinecone_search(query_embedding, faith_filter, top_k=5): # Using top_k=5 as requested
    ...
    try:
        logging.info(f"Querying Pinecone index '{pinecone_index_name}' filter={{'source': '{source_filter_value}'}}, top_k={top_k}")
        query_results = index.query(
            vector=query_embedding,
            filter={"source": source_filter_value},
            top_k=top_k,
        ...
        if query_results and query_results.get('matches'):
            for match in query_results['matches']:
                metadata = match.get('metadata', {}); text = metadata.get('text')
                reference = metadata.get('ref'); score = match.get('score', 0.0)
        ...
        logging.error(f"Error querying Pinecone: {e}"); return []
...
def find_relevant_scripture(transformed_query: str, faith_context: str) -> tuple[str | None, str | None]:
    ...
    if not index or not client: logging.error("Clients not init."); return None, None # Check module-level client
